    ...
    <div>
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-2 text-sm text-ink/60">Monitor your catalogue and keep product availability current.</p>
        </div>
      ...
      {error && (
        <p className="mt-6 rounded-md border border-red-200 bg-red-50 p-3 text-sm text-red-700">{error}</p>
      )}
      ...
      <div className="mt-7 grid gap-4 sm:grid-cols-3">
        {metrics.map(({ label, value, icon: Icon }) => (
          <div key={label} className="card p-5">
            ...
            <p className="mt-5 font-display text-3xl font-bold">{loading ? "…" : value}</p>
          </div>
        ))}
      </div>

      <div className="card mt-6 p-6">
        <h2 className="font-display text-xl font-bold">Catalogue actions</h2>
        <p className="mt-2 text-sm leading-6 text-ink/60">
          Add new stock, edit pricing and descriptions, or hide products from the public storefront.
        </p>
        <Link href="/admin/products" className="btn-secondary mt-5">Manage products</Link>
      </div>
    </div>
  );
}
