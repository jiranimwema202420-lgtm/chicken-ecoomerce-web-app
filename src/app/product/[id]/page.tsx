...
export default function ProductPage() {
  ...
  useEffect(() => {
    ...
    async function loadProduct() {
      try {
        ...
        if (active) setError("This product could not be loaded.");
      } finally {
  ...
  if (loading) {
    return <p className="section-shell py-12 text-sm text-ink/60">Loading product…</p>;
  }
  ...
  if (error || !product) {
    return (
      <div className="section-shell py-16 text-center">
        ...
        <p className="mt-2 text-sm text-ink/60">{error || "This product is no longer available."}</p>
  ...
  return (
    <div className="section-shell py-8 sm:py-12">
      <Link href="/#products" className="btn-ghost -ml-3 gap-2">
      ...
      <div className="mt-5 grid gap-8 lg:grid-cols-2 lg:gap-14">
        ...
        <div className="flex flex-col justify-center">
          <p className="eyebrow">{product.category || "Duka collection"}</p>
          ...
          <div className="mt-7 flex items-center gap-3">
            <span className={`h-2.5 w-2.5 rounded-full ${soldOut ? "bg-red-500" : "bg-forest"}`} />
            <p className="text-sm font-semibold">
              {soldOut ? "Currently out of stock" : `${product.stock} available`}
            </p>
          </div>
